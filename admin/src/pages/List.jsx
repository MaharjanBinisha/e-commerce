  ...
  return (
    <>
      ...
      <div className='flex flex-col gap-2'>
        ...
        {list.map((item, index) => (
          <div
            className='grid grid-cols-[1fr_3fr_1fr_1fr_1fr] items-center gap-2 py-1 px-2 border text-sm pr-4'
            key={index}
          >
            <img className='w-12' src={item.image[0]} alt='' />
            <p>{item.name}</p>
...
/>
      ...
      {/* Edit Form When a Product is Selected */}
{editProduct && (
  ...
  token={token}
  product={editProduct}
  setEditProduct={setEditProduct} // ✅ Ensure this is passed
/>


      <button onClick={() => setEditProduct(null)} className="mt-2 px-4 py-2 bg-red-500 text-white">
        Close
      </button>
    </div>
  </div>
